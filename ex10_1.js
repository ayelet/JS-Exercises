// ex10.1- Ayelet Danieli - Map vs. Foreach
...
let movies = (newReleases) => {
  let arr = [];
  newReleases.forEach((release) => {
    arr.push({ name: release.title, id: release.id });
  });
  return arr;
};
...
// use the map method.
function movies2(newReleases) {
    let arr = [];
    newReleases.map((p) => {
        arr.push({ name: p.title, id: p.id });
    });
    return arr;
}

console.log("\n=== Print the same thing using map ===");
console.log(movies2(newReleases));
